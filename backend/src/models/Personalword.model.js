...
const personalWordSchema = new mongoose.Schema(
  {
    ...
    // O'quvchi o'zi yozgan misollar (ixtiyoriy, cheksiz)
    examples: [{
...
// word + language + source bo'yicha — bir xil yozilish + bir xil til + bir xil manba
// lekin word xuddi bir xil bo'lsa ham translation farq qilsa ruxsat beriladi
// (masalan: "run" = yugurmoq, "run" = oqmoq — ikkisi ham saqlanadi)
personalWordSchema.index({ student: 1, source: 1, word: 1, language: 1 });
...
module.exports = mongoose.model('PersonalWord', personalWordSchema);// const mongoose = require('mongoose');
